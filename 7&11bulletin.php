<?php
    error_reporting(0);
    session_start(); #啟用 PHP session，讓我們可以存取 $_SESSION["id"]
    if (!$_SESSION["id"]) {
        echo "請先登入"; #如果沒有登入（沒有 $_SESSION["id"]），就顯示「請先登入」,3 秒後自動跳轉回登入頁 2.login.html
        ...
        echo "<table border=2><tr><td></td><td>佈告編號</td><td>佈告類別</td><td>標題</td><td>佈告內容</td><td>發佈時間</td></tr>";#標題列
